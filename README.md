...
## Звонок через «комнату» (без обмена ссылками)

Помимо обмена ссылками есть режим **«Позвонить (комната)»**: оба вводят один
и тот же код комнаты один раз, дальше звонок устанавливается сам.
Сигнальный сервер на выбор:

- облачный Firebase — см. [FIREBASE.md](FIREBASE.md);
- **свой сервер на Windows 7** в домашней сети — см. [WIN7_SERVER.md](WIN7_SERVER.md)
  (один Python-файл без зависимостей, запускается двойным кликом);
- TURN для звонков через мобильный интернет — см. [COTURN.md](COTURN.md).
